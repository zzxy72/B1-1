...
화면에 출력할 때는 뒤에 `%`가 붙는다.
-----
참고: awk는 텍스트 데이터를 가로(줄 단위)로 읽고, 세로(단어 단위)로 쪼개서 다루는 데 특화된 명령.
awk는 무조건 아래 공식 하나로 움직입니다.
awk '조건 { 행동 }' 파일이름
조건: 어떤 줄을 고를 것인가? (예: cpu로 시작하는 줄, 3번째 글자가 100 이상인 줄)
행동: 고른 줄을 가지고 무엇을 할 것인가? (예: 화면에 출력하기, 더하기 계산하기)
